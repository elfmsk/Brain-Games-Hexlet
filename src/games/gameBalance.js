...
export default () => {
  const balance = () => {
    ...
    const correctAnswer = (arg) => {
      ...
      if (max - min < stopBalance) { // Основное условие при выполнений которого вернется правильный ответ
        const result = {
          answer: sortArg.join(''),
    ...
    return correctAnswer(strMas(randomNumber)); //Вызываем функцию которая находит правильный ответ. Передаем ей рандомное число преобразованое в массив.
  };
